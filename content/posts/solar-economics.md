...
![Net and Cumulative Cashflows by Year](/images/posts/solar-economics/net-and-cumulative-cashflows-by-year.jpg "Cashflows from my planned system.")
...
Through this process, I also wanted to create something others could use, so I built [SolarCellsWork.com][4] (note: best viewed on desktop). I recommend playing around with the site to go along with my explanations here.
...
To make the model as helpful as possible, I allow users to supply a range of inputs and see the resulting distribution of possible outputs based on a Monte Carlo simulation. I was excited to add this feature since other calculators I found online present only one possible outcome rather than the range of possibilities. Big shout out to a product named [Casual][14] that made it easy to build this tool. The probabilistic nature of the simulation is why you might see slightly varying numbers in the screenshots in this post.

If you are using the model yourself, you can input whatever values you'd like for each input. To make it easier to use, I wanted to set some sensible defaults, which I'll explain.

## Determining the cash flows

Calculating the expected return is not as simple as adding up all the projected utility bill savings over the system's lifetime. That is because a dollar in the future is worth less than a dollar today i.e. the [time value of money][2].
...
Currently, in Illinois, where I live, I save money through what is known as ["net metering,"][21] which means I receive a credit for the electricity I send back to the grid at the same rate I get charged for it. Put another way, the meter runs backward when I produce more electricity than I am using. This arrangement makes it much simpler to model the economics of the system since I can think about my annual production and consumption offsetting each other 1 to 1. The net metering laws vary from state to state, which you can read more about [here](https://www.solarreviews.com/blog/the-state-of-net-metering-usa-2021).
...
#### Solar makes financial sense for me
...
On the flip side, a solar system is not a very liquid asset. The only feasible way to sell it would be to sell my house. It's not exactly clear if solar panels increase the value of a home, but the [evidence][20] (admittedly from sources with a vested interest in the answer) points to yes.
...
The federal subsidy will decrease from 26% to 22% in 2023 and then drop to 0% in 2024 for residential installations (it drops to 10% commercial and utility-scale). I am interested to see if Congress will renew the federal solar investment tax credit for residential installations and what happens if they don't.
...
#### Net Metering is Great Deal for Solar Owners

The electric grid has a large fixed cost to operate. The typical residential customers pays only [a small fixed cost on their bill and has all other utility costs built into their per kWh charge](https://www.engieimpact.com/insights/net-metering-becoming-controversial?Most%20residential%20and%20smaller%20customers,%20however,%20pay%20only%20a%20small%20fixed%20monthly%20charge%20with%20all%20other%20utility%20costs%20built%20into%20their%20charge%20per%20kWh.#:~:text=small%20fixed%20monthly-,charge,-with%20all%20other).

At the extreme, net metering can result in purchasing little to no power from the grid. In that case, the customer is not contributing to the fixed costs required to maintain the grid. These costs need to get paid, so they result in higher rates from non-net metered customers.

Even though the costs of solar have gone down, the amount solar owners get paid for their generation has stay pegged to the retail rate of electricity.

There are other alternatives out such as ["value of solar"](https://ilsr.org/wp-content/uploads/2014/04/MN-Value-of-Solar-from-ILSR.pdf), where utilities pay a market price based on the value of the solar energy to the grid and commit to that price for 25 years.
...
[21]: https://www.seia.org/initiatives/net-metering
